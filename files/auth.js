...
// accounts.email. Writing accounts.email directly would break the User ID
// mapping — that was the bug this design replaced.
//
// The current password is required first. An email address is account
// recovery: whoever controls it can reset the password. Changing it is
// therefore as sensitive as changing the password itself, and gets the same
// confirmation (D-05 reasoning applied to D-07).
...
// The pending address, if any. Supabase exposes it as new_email on the user
// object while a change is awaiting confirmation.
...
export async function resendEmailChange(newEmail) {
  ...
  if (error) {
    // Supabase enforces its own resend rate limit server-side; surface it in
    // plain words rather than the raw message.
